...
class Vehicle():
    def __init__(self, address: str=None, baud: int=57600, autoreconnect: bool=True, drone_id: int=1, on_flight: bool=True):
        try:
            ...
            if on_flight:
                ...
                self.request_message_interval("GLOBAL_POSITION_INT", 1, self.drone_ids)
                self.request_message_interval("MISSION_CURRENT", 4, self.drone_ids)
    ...
    def get_pos(self, drone_id: int=None):
        ...
        try:
            ...
            start_time = time.time()
            while True:
                message = self.vehicle.recv_match(type='GLOBAL_POSITION_INT', blocking=False)
    ...
    def get_yaw(self, drone_id: int=None):
        ...
        try:
            ...
            start_time = time.time()
            while True:
                message = self.vehicle.recv_match(type='ATTITUDE', blocking=False)
    ...
    def on_location(self, loc, drone_loc=None, seq: int=0, sapma: int=1, drone_id: int=None):
        if drone_id is None:
            drone_id = self.drone_id
        ...
        try:
            ...
            if drone_loc == None:
                drone_loc = self.get_pos(drone_id=drone_id)

            if seq != 0:
                if abs(drone_loc[0] - loc[0]) <= self.DEG*sapma and abs(drone_loc[1] - loc[1]) <= self.DEG*sapma and self.get_miss_wp(drone_id=drone_id) == seq:
                    return True
                return False
            else:
                if abs(drone_loc[0] - loc[0]) <= self.DEG*sapma and abs(drone_loc[1] - loc[1]) <= self.DEG*sapma:
                    return True
                return False
        except Exception as e:
            return e
