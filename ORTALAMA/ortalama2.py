...
while not video_handler.udp_connected.is_set():
    if time.time() - timer >= 2:
        print("UDP Görüntüsü bekleniyor...")
        timer = time.time()
...
try:
    ...
    vehicle.set_guided_speed(speed=1.5, drone_id=DRONE_ID)
    ...
    while not stop_event.is_set():
        ...
        if not image_queue.empty() and yuk_birakildi == False:
            # Hedef ilk bulunduğu anda durması için
            if durdu == False:
                durdu = True
                print("Hedef Bulundu duruluyor")
                vehicle.move_drone((0, 0, 0), drone_id=DRONE_ID)
                timer = time.time()
                while not stop_event.is_set():
    ...
    if yuk_birakildi:
        ...
        vehicle.set_mode(mode="GUIDED", drone_id=DRONE_ID)
        vehicle.go_to(loc=home_pos, alt=ALT, drone_id=DRONE_ID)
        print(f"{DRONE_ID}>> kalkış konumuna ({home_pos}) dönüyor...")

        start_time = time.time()
        while vehicle.on_location(loc=home_pos, seq=0, sapma=1, drone_id=DRONE_ID) == False:
