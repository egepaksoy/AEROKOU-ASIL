...
try:
    ...
    timer = time.time()
    while not stop_event.is_set():
        drone_loc = vehicle.get_pos()
        drone_yaw = vehicle.get_yaw()

        if time.time() - timer >= 5:
            print(f"{DRONE_ID}>> Göreve Devam Ediyor")
            timer = time.time()
        
        if vehicle.on_location(loc=current_loc, drone_loc=drone_loc, seq=0, drone_id=DRONE_ID):
        ...
        if algilandi.is_set():
            ...
            obj_pos = get_position(camera_distance=distance.get(), total_yaw=((camera_yaw + drone_yaw) % 360), current_loc=drone_loc)
